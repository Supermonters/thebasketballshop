...
function register() {
  ...
  if (counterObj.NguoiDung === undefined) {
    var a = 1;
  } else {
  ...
  if (localStorage.length > 0) {
    for (let i = 0; i < localStorage.length; i++) {
      try {
        let taiKhoanTrenLocal = JSON.parse(
...
function login() {
  if (localStorage.getItem("TaiKhoanDangNhap") !== null) {
    alert(
      "Bạn hiện đang đăng nhập,vui lòng đăng xuất để đăng nhập vào tài khoản khác"
    );
  } else {
    var taiKhoanGhiVao = {
      name: document.getElementById("username").value,
    ...
    if (taiKhoanGhiVao.name === "" || taiKhoanGhiVao.pass === "") {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        var nguoiDungNhapTuLocal = JSON.parse(
          localStorage.getItem(localStorage.key(i))
        );
