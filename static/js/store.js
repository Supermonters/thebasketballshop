(async () => {
    const fits = document.getElementsByClassName("filterable-items")[0];
    const f = await fetch("/productlist");
    const data = (await f.json()).products;
    for (let i = 0; i < data.length; i++) {
        fits.innerHTML +=
            `<div class="filterable-item ${data[i].filter}">
            <button>
